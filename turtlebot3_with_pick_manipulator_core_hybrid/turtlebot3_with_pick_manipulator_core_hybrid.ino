...
static int initail_state = false ;
...
void setup()
{
  ...
  motor_driver.init();
  initail_state = joint_driver.init();
...
void goalJointPositionCallback(const sensor_msgs::JointState& goal_joint_position_msg)
{
  ...
  for (int index = 0; index < JOINT_NUM; index++){
    ...
    DEBUG_SERIAL.print(goal_joint_position[index]);
    DEBUG_SERIAL.print(",");
  }
  //DEBUG_SERIAL.println("");

  DEBUG_SERIAL.print( "............" );
  DEBUG_SERIAL.print( get_joint_position[0] );
  DEBUG_SERIAL.print(",");
  DEBUG_SERIAL.print( get_joint_position[1] );
  DEBUG_SERIAL.println("");
