...
bool setupMotorJoint1(void);
void testMotor(uint8_t id);
void testMotorJoint(uint8_t id);
...
void loop()
{
  ...
  if (CMD_SERIAL.available())
  {
    ...
    if (ch == '5')
    {
      ...
      if (requestConfirm() == true)
      {
        CMD_SERIAL.println("setup.... joint1");
    ...
    {
      flushCmd();
      CMD_SERIAL.println("test.... joint1");
      testMotorJoint(3);
    }
...
void drawTitle(void)
{
  ...
  CMD_SERIAL.println("5. setup  manipulator joint1 motor");
  CMD_SERIAL.println("6. test   manipulator joint1 motor");
  CMD_SERIAL.print(">> ");
}
...
bool setupMotorJoint1(void)
{
  CMD_SERIAL.println("Setup Motor Joint1...");
  ...
  else
  {
    ...
    write(portHandler, packetHandler2, tb3_id, 10, 1, 0);
    //Drive Mode  set Normal mode(‘0’) 
    write(portHandler, packetHandler2, tb3_id, 11, 1, 3);
    //Operating Mode Velocity Position Control Mode ( default )
    CMD_SERIAL.println("    ok");    
  }
}
...
void testMotorJoint(uint8_t id)
{
  ...
  else
  {
    ...
    pre_time = millis();
    //write(portHandler, packetHandler2, id, 104, 4, 100);
    write(portHandler, packetHandler2, id, 116, 4, 2048);
    while(1)
    {
      if (CMD_SERIAL.available())
      {
      ...
      if (millis()-pre_time > 1000)
      {
        ...
        {
          //write(portHandler, packetHandler2, id, 104, 4, 100);
          write(portHandler, packetHandler2, id, 116, 4, 1024);
        }
      }
    }
    //write(portHandler, packetHandler2, id, 104, 4, 0);
